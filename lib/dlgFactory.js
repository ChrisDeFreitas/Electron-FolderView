/*
	...
	///hvr dialog
	- popup dialog when mouse hovered over parent element
	dlgFactory.hvr({
		parent:ctrl						//parent may be null; hover child's current location
	, child:btnlist
	//, destroyOpacity:0.2	//child.onClose: lower opacity instead of hiding
...
var dlgFactory = {
	...
	close: function(dlgid, force){
		...
		if(options){
		  if(options.onClose!=null)
				...
				dlg.style.display = 'none'
			if(options.canDestroy==false && force===false) return
			if(dlg.dlg.id)
	...
	//tac2  - tack html to screen or control
	tac2: function(options){
		if(this.inited===false) this.init()
		//console.log(options)
		...
		if(options.focusCtrl!==undefined) options.to.focusCtrl=null
		...
		if(options.to && options.to.fadeDelay!==null){
			...
			, child:div
			, fadeDelay:options.to.fadeDelay
			})
		}
	...
	fadeIn:function(div, delay){		//delay=0: no auto FadeOut
		...
		if(delay==0)
			return true
		div.dlg.timer = window.setTimeout(function(){	//pause for options.delay then hide div
			div.dlg.timer=null
			if(div.dlg.focused && div.dlg.focused===true){		//don't destroy while focused
				div.dlg.leaveFadeOut=true
				return
			}
			dlgFactory.fadeOut(div)
		}, delay)
		return div.dlg.timer
	},
	fadeOut:function(div, delay, force, event){
		if(div.dlg===undefined) div.dlg = {}
		if(div.dlg.timer!==null){
			window.clearTimeout(div.dlg.timer)
			div.dlg.timer=null
		...
		function local_destroyTimeout(){
			div.dlg.timer = window.setTimeout(function(){
				div.dlg.timer = null
				//div.dlg.destroy(force, event)
				//if(force===false && div.dlg.timer!=null) return			//either delay or destroy timers
				if(force===false && div.dlg.leaveFadeOut===true){
					div.dlg.leaveFadeOut=false
					dlgFactory.fadeOut(div,0,false,event)
					return
				}
				if(force===true
				||(div.dlg.canDestroy!=null && div.dlg.canDestroy===true)){
					if(div.dlg.timer!=null)		//force timeout to end
						window.clearTimeout(div.dlg.timer)
					div.dlg.timer=null
					document.body.removeChild(div) //div.remove()
				} else
				if(div.dlg.destroyOpacity!=null){			//leave on screen, at low opacity
					div.style.opacity = div.dlg.destroyOpacity
					return
				}
				div.style.display='none'
			},300)
...
dlgFactory.hvr = function(options){
	...
	if(options.destroyOpacity===undefined) options.destroyOpacity=null
	if(options.fadeDelay==null) options.fadeDelay=hvrFadeDelay

	options.parent = ui.calc.ctrlFromStr(options.parent)
	//console.log('hvr parent',  options.parent)
	options.child = ui.calc.ctrlFromStr(options.child)
	if(options.parent.dlg==null) options.parent.dlg={ options:{} }
	if(options.child.dlg==null) options.child.dlg={ options:{ canDestroy:false }}

	var parent = options.parent
	var child = options.child
	parent.dlg.hvrChild = options.child
	child.dlg.hvrParent = options.parent
	child.dlg.destroyOpacity = options.destroyOpacity
	child.dlg.fadeDelay = options.fadeDelay

	if(parent != null) {		//parent===null => hover in place
		parent.addEventListener('mouseenter', (event) => {
			dlgFactory.fadeIn(child,0)
		})
		//parent.addEventListener('mouseleave', (event) => {
		parent.onmouseleave = function(event){
			//console.log('pa.event.relatedTarget.id',event.relatedTarget.id)
			if(event.relatedTarget.id===parent.id
			||event.relatedTarget.id===child.id
			||event.relatedTarget.parentNode.id===child.id) return
	...
	child.onmouseenter = function(event){
		...
		//if(child.dlg.timer!==null)return
		//dlgFactory.fadeIn(child)
	}
	child.onmouseleave = function(event){
		//console.log('ch.event.relatedTarget.id',event.relatedTarget.id)
		child.dlg.focused=false
		if(event.relatedTarget.id===parent.id
		||event.relatedTarget.id===child.id
		||event.relatedTarget.parentNode.id===child.id) return
...
dlgFactory.btnList = function(options){
	...
	var btnList = dlgFactory.tac2({
		to:{ ctrl:options.parent
			 , append:(options.parentAppend ?options.parentAppend :false)
			 , from:(options.parentFrom ?options.parentFrom :'centerright') }
	, from:(options.from ?options.from :'centerleft')
	, canDestroy:(options.canDestroy ?options.canDestroy :false)
	, createHidden:(options.createHidden ?options.createHidden :false)
	,	html:html
	, attrib: (options.attrib ?options.attrib :null)
	, maxHeightNone:options.maxHeightNone
	, maxWidthNone:options.maxWidthNone
	, onClose:function(div, force){
			if(options.onClose) options.onClose(div)
