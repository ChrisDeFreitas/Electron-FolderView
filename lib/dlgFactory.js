/*
	...
	- drag title bar
	- disabled feature:  dblClick title bar to close
...
var dlgFactory = {
	...
	create: function(options) {
		...
		if(options.focusId===undefined) options.focusId=null			//expected format: #id or .id
		if(options.headerHidden===undefined) options.headerHidden=false
		if(options.rollup==null) options.rollup=false				//rollup dialog so only title is visible
		if(options.id===undefined) options.id=null								//override default id
		...
		//other options: title, top, left, width, height

		var id = (options.id !== null ?options.id :'dlg'+(++this.lastid) ),
		 	btnhtml = '',
		 	btnCloseHide=false
		if(options.buttons){
		...
		var	left=(options.left ?";left:"+options.left :''),
			...
			hdrclick =( options.onHeaderClick===null && options.rollup === false ?'' :`onclick="dlgFactory.hdrClick('${id}')"`)
			//ondblclick='dlgFactory.close(\""+id+"\")'
			html = "\
		<div id=hdr class=dlgHeader draggable=true ondragstart='dlgFactory.dragStart(event)' ondragend='dlgFactory.dragEnd(event)'	\
		 "+hdrclick+" style='display:"+(options.headerHidden===false ?'block' :'none')+"'> "
			+options.title+" </div> \
		...
		<div id=ftr"+id+" class=dlgFooter style='display:"+(options.footerHidden===true ?"none" :"block")+"' >"
			...
			if(options.focusId != null){	//focus default ctrl: focusId
	...
	hdrClick: function(dlgid){
		...
		if(dlg.dlg.options.onHeaderClick != null)
			dlg.dlg.options.onHeaderClick(dlg)
		else
		if(dlg.dlg.options.rollup === true){

			if(dlg.dlg.minimized===false){
				dlg.dlg.minimized=true
				dlg.dlg.paddingLast = dlg.style.padding
				dlg.dlg.borderLast = dlg.style.border
				dlg.dlg.RollupOffset = dlg.querySelector('#hdr').offsetTop +1

				dlg.style.padding = '0'
				dlg.style.border = '0'

				dlg.style.top = (dlg.offsetTop +dlg.dlg.RollupOffset)+'px'
				dlg.style.left = (dlg.offsetLeft +dlg.dlg.RollupOffset)+'px'
				...
				dlg.style.padding = dlg.dlg.paddingLast
				dlg.style.border = dlg.dlg.borderLast
	...
	msg: function(msg, title, options){
		...
		if(options.focusId==null) options.focusId = '#btnClose'
		...
		options.attrib.style.zIndex=1600
		options.body=msg
	...
	bbl: function(msg, delay){
		...
		if(delay===undefined) delay = bblDur	//6000
	...
	init: function(){
		...
		var rules = [
			...
			".dlgButton{ background:#0d3; border:0; border-radius:0.25em; cursor:pointer; color:#fff; font-size:0.8em; height:2em;}",
...
dlgFactory.hvr = function(options){
	...
	if(options.hoverDelayMs===undefined) options.hoverDelayMs=0
	...
	if(options.showOnParentClick===undefined) options.showOnParentClick=false
	...
	if(parent != null) {		//parent===null => hover in place
		...
		parent.dlg.timeout = null		//for hoverDelayMs
		parent.dlg.onMouseEnter = function(){
			...
			if(opts.onShow!=null){																	//confirm?
				if(false===opts.onShow(opts,event)) return true
			}
			dlgFactory.fadeIn(child,opts.fadeInOpacityMs)						//display child
			return true
		}
		parent.addEventListener('mouseenter', (event) => {
			if(opts.hoverDelayMs == 0){			//no delay; use default handler
				return parent.dlg.onMouseEnter()
			}
			//create timer to delay display of child
			parent.dlg.timeout = window.setTimeout(function(){
					if(parent.dlg.timeout != null){ //mouse still inside control
						let result = parent.dlg.onMouseEnter()
						window.clearTimeout(parent.dlg.timeout)
						parent.dlg.timeout = null
						parent.dlg.childVisible = true
						return result
					}
				},
				opts.hoverDelayMs
		...
		parent.addEventListener('mouseleave', (event) => {
			if(parent.dlg.timeout != null) {		//remove timeout
				window.clearTimeout(parent.dlg.timeout)
				parent.dlg.timeout = null
			}
			//if hoverDelayMs > 0 this is not necessary, but safe
			dlgFactory.fadeOut(child,opts.fadeOutOpacity, opts.fadeOutOpacityMs, opts.fadeOutWaitMs)
		})
		if(options.showOnParentClick===true){
			parent.addEventListener('click', (event) => {
				if(parent.dlg.timeout != null) //mouse inside control; child not visible
					window.clearTimeout(parent.dlg.timeout)
				if(child.style.display==='none'){		//show child
					parent.dlg.onMouseEnter()
				}
				else{			//hide child
					dlgFactory.fadeOut(child,opts.fadeOutOpacity, opts.fadeOutOpacityMs, opts.fadeOutWaitMs)
				}
...
dlgFactory.tac2 = function(options){	//tac2  - tack html to screen or control
	...
	if(typeof options.html == 'string'){
		...
		div.id = (options.id ?options.id :'dlg'+(++this.lastid))
