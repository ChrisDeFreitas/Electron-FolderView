...
function dlgSFTP(){
	...
	this.connect = function(){
		if(me.conn != null){
			dlgFactory.bbl('Already connected to: '+ me.uri, sftpbbldur)
			return
		}
		...
		.on('ready', function(){
			dlgFactory.bbl('Connected: '+me.uri, sftpbbldur)
	...
	this.remoteDelete = function(item, remotepath, stepfunc, deldonefunc){
		...
		if(item.isFolder!=true){
			this.sftpstream.unlink(remotepath, function(err){
				if(err && err != 'Error: No such file'){
					...
					if(deldonefunc != null)	deldonefunc(err)
				}else{
					if(err && err == 'Error: No such file')
					...
					if(deldonefunc != null)	deldonefunc(null)
				}
			})
			return
		}
	...
	this.remoteDownloadFolder = function(item, remotepath, localpath, stepfunc, donefunc){
		//create new fold
		remotepath = remotepath + '/' + item.filename
		//invalid filename chars in windows: :/\\*^"<>|
		localpath = localpath + '\\' + item.filename.replace(/[:/\\*^"<>|]/,'_')
		var fs = require('fs');
			...
			, local_deletecb = function(err)
				{	if(err){
					...
					if(dlgsftp.auto === true){
						...
						if(deletecount === 0){
							//if(idx === arr.length -1){
							dlgFactory.bbl('Auto: Delete done.', sftpbbldur)
							console.log('Auto: Delete done.')
							if(dlgsftp.autoAfterDelete)	dlgsftp.autoAfterDelete()
						}
					}
				}
		dlgsftp.remoteDelete(item, dlgsftp.remotePath, local_stepfunc, local_deletecb)
	}
}
dlgSFTP.downloadClick = function(event){
	...
	if(dlgsftp.auto === true)
		dlgFactory.bbl(`Auto: ${arr.length} items to download.`, sftpbbldur)
	else {
	...
	function local_launchDownload(){
		...
		,function(err){
			...
			dlgFactory.bbl('Downloaded: '+item.filename, sftpbbldur)
			++donecount
	...
	dlgsftp.autoAfterLoad = function(){
		if(dlgsftp.list.length==0){
			...
			if(pnl.querySelector('#lblExit input').checked === true){
				...
				dlgFactory.bbl('Auto: Done, no remote items found.', sftpbbldur)
			}
			return
		}
		if(pnl.querySelector('#lblDownload input').checked === true){
			...
			dlgFactory.bbl('Auto: Done.', sftpbbldur)
			dlgsftp.auto = false
		}
	}
	dlgsftp.autoAfterDownload = function(){
		if(pnl.querySelector('#lblReload input').checked === true
		&& dlgsftp.options.localPath == ui.args.path){
			//dlgFactory.bbl('Auto: reloading local folder after download.', sftpbbldur)
