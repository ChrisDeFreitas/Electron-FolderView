ui = {
	...
	args: {
		...
		fullscreen:false,
		height:0,
		iconsOnly:false,			//display file icons instead of HTML image/video/audio controls
		isElectron:true,
		layout:'cols',
...
function gridLoad(layoutMode, extFilter){
	...
	if(ui.args.shuffle==true){
		...
		itemselected=null
		dlgFactory.bbl('Shuffling folder items...', 2000)
	...
	for(var idx in items){	//define grid items
		...
		if(io===true || obj.mediaType === 'unknown'){
			let src = null
		...
		if(layoutMode !== 'vert') {
			...
			html += `<table id="tgi${obj.pid}" class='grid-item'><tr class=vertTr>
...
function gridSort(tableId, col, rowStart, numericCol) {
