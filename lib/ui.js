...
ui.calc = {
	...
	execSpawn: function(command, args) {
...
//  	console.log('openPhotoSwipe(): event.ctrlKey')
	...
	gallery.dlg = {
		dlgParent:null			//allows synchronized destroy event
	}
...
function itemDrop(event){
	let tpid = controlIdToPid(event.target.id),
			obj = items[tpid],
			dest = obj.path,
			pids = event.dataTransfer.getData("text")
	...
	if(obj.isDirectory == true){
		...
		renderer.bulkOps( pids, dest, true )
	}
}
...
function selectItems(list){
	//asume list is one of: pid, 'pid1,pid2', [pid1, pid2]; undefined = all items
	...
	if(list === undefined){			//select all
		if(selectListCnt() > 0)			//toggle selection
			list = []
		else
