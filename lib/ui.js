...
ui.calc = {
	pulse: function(maxMs, fps, cb){
		...
		var msPerFrame = Math.round(1000/data.fps)
		var cbResult = null
		///
		var pulseFunc = function(ts) {
			...
			data.fpsCalc= Math.round(1000/(data.ts -data.tsLast))
			data.callNo++
			//data.fno = Math.floor( (data.ms/msPerFrame) % data.fmax)	//use ms incase time interrupted
			data.fno = Math.floor( (data.ms/msPerFrame) % data.fps)	//use ms incase time interrupted
...
//			height=${height}
		...
		if(layout==='vertical') {	//attach div with file data
			...
			if(ui.args.isElectron===true)
				ss+=`<br>Name: ${obj.basename}`
			else{
				if(obj.title!==undefined)
					ss+=`<br>Title: <a href="${obj.src}" target="_new">${obj.title}</a>`
				else
					ss+=`<br>src: <a href="${obj.src}" target="_new">${obj.src}</a>`
			}
			if(obj.owner!==undefined)
					ss+=`<br>Owner: <a href="${obj.url}" target="_new">${obj.owner}</a>`
			if(imgtypes.indexOf(obj.type) >= 0)
				ss+=`<br>WxH: ${obj.w} x ${obj.h}`
			if(obj.size!==undefined)
				ss+=`<br>Size: ${Math.round(obj.size/1024)}kb`
			if(obj.date!==undefined)
				ss+=`<br>Modified: ${new Date(obj.date).toString()}`
			if(dsc!='')
				ss+=`<br><br>${dsc}`
