...
var imgCaptionOptions = {
	canDestroy:false
, ctrl: null
, delay:10000
, destroyOpacity: 0.05
, from:'bottomright'
, to:null		//updated with each call {x:window.innerWidth-60, y:window.innerHeight-40}
, width:333
, attrib:{
		innerHTML:null
	...
	, onclick:function(){
			imgCaptionOptions.ctrl.style.display='none'
		}
	}
}
...
ui.init = function(){
	...
	document.getElementById('btnGalleryDur').innerHTML = (galleryDelay/1000)+'s'
	...
	for(var idx in items){	//define grid items
		...
		if(layout==='vertical') {	//attach div with file data
			...
			if(obj.size!==undefined && obj.size>0)
				ss+=`<br>${Math.round(obj.size/1024/1024)}mb`
...
function openPhotoSwipe(pid) {
	...
	gallery.listen('destroy', function() {
		...
		imgCaptionOptions.ctrl.DLGDestroyFunc(true)
	...
	*/
		imgCaptionOptions.html=`<a href="${obj.url}" target="_new">${obj.basename}</a>
			<br>${obj.h} x ${obj.w}
			<div style="text-align:left">
			${dsc}
			</div>`
		imgCaptionOptions.to = {x:window.innerWidth-40, y:window.innerHeight-20}	//incase it changes
		imgCaptionOptions.delay = (galleryDelay/2>10000 ?10000 : Math.round(galleryDelay/2))
		imgCaptionOptions=dlgFactory.tac(imgCaptionOptions)
		if(imgCaptionOptions.attrib!=null)
			imgCaptionOptions.attrib=null		//only needs to be assigned once
	})
	gallery.init()
}
function galleryImageScale(wheelEvent, boo) {
	...
	if(wheelEvent==null)
		...
		//needs more to work properly with photoswipe
		var pos = ui.calc.xyAbs(wheelEvent.target)
		pos.left += wheelEvent.clientY
		pos.top += wheelEvent.clientX
		gallery.zoomTo(sc, {x:pos.left,y:pos.top}, 0)
...
function galleryDur(){	//get slideshow delay
	...
	function local_update(){
		galleryDelay = tmpdlg.querySelector('#inpDelay').value*1000
		document.getElementById('btnGalleryDur').innerHTML = (galleryDelay/1000)+'s'
...
function openYoutube(pid){
	...
	var html = `<iframe frameborder="0" allowfullscreen
				...
				style="height:${obj.h}px;width:${obj.w}px;margin:0 auto;"></iframe>`
		var hh = Math.floor(window.innerHeight-800)/2
		dlgFactory.create({
			title:obj.basename
		, headerHidden:true
		,	focusId: '#btnDlgClose'
