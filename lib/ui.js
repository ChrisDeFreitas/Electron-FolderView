...
//			ondblclick= `itemDblClick(${obj.pid}, event)`,
				title			= obj.basename
...
//		}
			...
			title += `, ${ui.calc.bytesToStr(obj.size)}`
			childhtml = `<img id="unk${obj.pid}"	src="${obj.src}" alt="${obj.basename}"
...
//				ondblclick="${ondblclick}"
	...
	if(cnt===items.length)
		dlgFactory.bbl(`${cnt} items`, 2000)
	else
		dlgFactory.bbl(`${cnt}/${items.length} displayed`, 2000)
}
...
//	console.log('itemClick', event.target.id, event.detail, event.timeStamp)
  if(event.ctrlKey===true){		//itemSelected list handling
	...
	if(event.detail === 2){			//double-click
		//console.log('Double-click', event.target.id, grid.dlg.pidClicked)
		clearPidClicked()
		...
		grid.dlg.pidClickedTimeout = window.setTimeout(		//delay click action in case there is a dblclick
			function(pidClicked){
				//console.log('pidClickedTimeout', grid.dlg.pidClicked, pidClicked)
