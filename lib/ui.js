...
ui.var = {							//store dynamic vars here
	...
	execQueCnt:0,			//number of executions occurring
	execQueList:[],		//list of commands waiting to be executed

	recentListMax:25		//maximum items to appear in the recent list
}
...
ui.calc = {
	...
	execAsync: function(command, callback) {
		...
		const results = childexecAsync(command, function(error, stdout, stderr){
			if (error)
	    	console.error(`ui.calc.execAsync()\n  error: [${error}]\n  stdout: [${stdout}]\n  stderr: [${stderr}]`);
    	if(callback != null)
...
function gridLoad(layoutMode, extFilter){
	if(isotope!==null){
		ui.reset()
	}
	...
	for(var idx in items){	//define grid items
		...
		//
//		let width =(obj.w_z != undefined ?obj.w_z :obj.w),
//				height=(obj.g_z != undefined ?obj.g_z :obj.h),
		let width=obj.w, height=obj.h, src=obj.src
//		if(obj.type==='youtube'){
//			src=obj.src_z
//			height=obj.h_z
//			width=obj.w_z
...
//		}
		...
		height=Math.floor( height *scale)		//apply user's scale argument
		...
		if(obj.isDirectory===true){
			childhtml = `<img id="fld${obj.pid}"	src="${src}" alt="${obj.basename}"
				style="margin:2em auto 0 auto;padding:0 2em;"
				></img>`
		} else
		if(obj.mediaType === 'image') {
			onclick 	= `openPhotoSwipe(${obj.pid}, event)`
			ondblclick= ''
			title 	 += `, ${obj.w} x ${obj.h}, ${ui.calc.bytesToStr(obj.size)}`
			childhtml = `<img	id="img${obj.pid}"	src="${src}" alt="${obj.basename}"
				style="height:${height}px;width:${width}px;display:${layout==='vertical'?'inline-table':'block'};"
				></img>`
		}	else
		if(obj.mediaType === 'video'){
			title += `, ${ui.calc.bytesToStr(obj.size)}`
			childhtml = `<video controls preload='metadata'
				id="vid${obj.pid}" src="${src}" type="video/${obj.type.substr(1)}"
		...
		if(obj.mediaType === 'svg'){
			onclick = `openPhotoSwipe(${obj.pid}, event)`
			title	 += `, ${ui.calc.bytesToStr(obj.size)}`
			childhtml = `<img id="svg${obj.pid}"	src="${src}" alt="${obj.basename}"
...
//			onclick		= `openYoutube(${obj.pid}, event)`
//			title		 += `, ${ui.calc.bytesToStr(obj.size)}`
//			childhtml = `<img id="img${obj.pid}"	src="${src}" alt="${obj.basename}"
//				style="height:${height}px;width:${width}px;display:${layout==='vertical'?'inline-table':'block'};"
//				></img>`
//		}
		else {		//obj.mediaType === 'unknown'
			title += `, ${ui.calc.bytesToStr(obj.size)}`
		...
		if(['image','svg'].indexOf(obj.mediaType) < 0){ //add caption at bottom for non-images
...
//  	console.log('openPhotoSwipe(): event.ctrlKey')
	...
	for(var idx in workitems) {	//scale images
		...
		if(item.mediaType !== 'image')
			continue
		if(item.isDeleted && item.isDeleted===true) continue

		if(defaultPid===0 && pid==item.pid)
...
function itemClick(pid, event){
  if(event.ctrlKey===true){		//itemSelected list handling
		...
		itemSelect(pid)		//will empty selectList
	}
}
...
function selectListRemove(pid){
	...
	if(idx >= 0){
		...
		//console.log('selectListRemove',pid, ui.var.selectList)
	}
}
