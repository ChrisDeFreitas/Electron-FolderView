...
//	console.log(`shuffle=`, ui.args.shuffle)
	...
	var html = '', title='', cnt=0, layout=null, defaultImageNum=null
	...
	for(var idx in items){	//define grid items
		...
		html += `<div onclick="itemClick(${obj.pid}, event);" class='grid-item' id="obj${obj.pid}" title="${obj.basename}"
		...
		if(layout==='vertical') {	//attach div with file data
			...
			if(imgtypes.indexOf(obj.type) < 0 && obj.type!=='.svg'){ //obj.basename at bottom
				html += `<div style='padding:0; margin:0 auto; overflow:hidden; text-overflow:ellipsis;' title="${title}">${obj.basename}</div>`
			}
		}
...
//		if(defaultPid===0 && pid==idx) defaultPid = galleryItems.length
		if(defaultPid===0 && pid==item.pid) defaultPid = galleryItems.length
		galleryItems.push({
			src: item.src,
