...
videoDl.init = function(options) {
	...
	html = `<label class="divBulkOptions" style="display:block;margin:1em auto; width:94%;">URL: <input id=dlgVideoDlUrl type=text value="${options.url}" onfocus='this.select()' size=40>
...
videoDl.getInfo = function(callback){
...
//		videoDl.cfg.title = info.title
		videoDl.cfg.title = info.player_response.videoDetails.title
