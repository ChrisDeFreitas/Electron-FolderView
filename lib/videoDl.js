...
videoDl.getInfo = async function(callback){
	...
	try{  // promise.catch fails to capture errors
		...
		.then(info => { //, {}, function(err, info){
			...
			html = `Video Title: &nbsp; <input type=text id=vidTitle style="font-weight:normal; width:81%; " value="${videoDl.cfg.titleO}" title="Change to rename downloaded video file"><br>
