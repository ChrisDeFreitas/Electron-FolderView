	    ...
	    submenu: [
	        ...
	        label: 'System Info', accelerator:'F2',
					click: function(item, focusedWindow){
						...
						else dt = dt = (Math.round( dt /24 *10) /10)+' days'
						var str = `
							...
							<br>`
							+ JSON.stringify(process.getSystemMemoryInfo()).replace(/,/g, '<br>').replace(/{/, '<b>System Memory(KB)</b><br>').replace(/}/g, '<br><br>')
							+ JSON.stringify(process.getProcessMemoryInfo()).replace(/,/g, '<br>').replace(/{/, '<b>Process Memory(KB)</b><br>').replace(/}/g, '')
						if(dlgSysInfo==null)
							dlgSysInfo = dlgFactory.msg(str, 'System Info')
						else{
							dlgFactory.close(dlgSysInfo, true)
							dlgSysInfo=null
						}
					}
				}/*,{
...
function libraryLoadDefault(){
	...
	result.push(['home',app.getPath('home').replace(/\\/g,'/')+'/'])
	result.push(['desktop',app.getPath('desktop').replace(/\\/g,'/')+'/'])
	result.push(['documents',app.getPath('documents').replace(/\\/g,'/')+'/'])
	result.push(['downloads',app.getPath('downloads').replace(/\\/g,'/')+'/'])
	result.push(['music',app.getPath('music').replace(/\\/g,'/')+'/'])
	result.push(['pictures',app.getPath('pictures').replace(/\\/g,'/')+'/'])
	result.push(['videos',app.getPath('videos').replace(/\\/g,'/')+'/'])
	return result
	/*
	console.log('home\n',app.getPath('home'))
	console.log('appData\n',app.getPath('appData'))
