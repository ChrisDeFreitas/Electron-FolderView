	...
	var template = [
	  {
	    ...
	    submenu: [
	        ...
	        label: 'System Info', accelerator:'F2',
					click: function(item, focusedWindow){
						var str = `<b>Node</b> v${process.versions.node}<br>
							<b>Chrome</b> v${process.versions.chrome}<br>
							<b>Electron</b> v${process.versions.electron}<br><br>`
							+ JSON.stringify(process.getSystemMemoryInfo()).replace(/,/g, '<br>').replace(/{/, '<b>System Memory</b><br>').replace(/}/g, '<br><br>')
							+ JSON.stringify(process.getProcessMemoryInfo()).replace(/,/g, '<br>').replace(/{/, '<b>Process Memory</b><br>').replace(/}/g, '')
						if(dlgSysInfo==null)
