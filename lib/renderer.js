...
*/
	  ...
	  for(let sub of subs){
			...
			tmp += `<tr>
					<td><b>${fldr}</b></td>
					<td align=right>${sub.subfolders.length.toLocaleString()}</td>
					<td align=right>${sub.totalitems}</td>
					<td align=right>&nbsp; ${ui.calc.bytesToStr(sub.size)}</td>
				</tr>`
		}
		if(tmp != '')
			result += '<table><tr style="font-style:italic"><td>Folder</td><td>Items</td><td>&nbsp;Total Items</td><td>&nbsp;Total Size</td></tr>'
							+ tmp
							+ '</table>'
