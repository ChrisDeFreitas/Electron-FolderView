	...
	var template = [
	  {
	    ...
	    submenu: [
	        ...
	        label: 'Find Item', accelerator: 'Ctrl+F',
					click: function(item, focusedWindow){
						findItem()
					}
	      },{
			...
			submenu: [
				{ label: 'Folders', accelerator:'Alt+Shift+F',
					...
					{	label: 'Default', type:'radio', checked:(ui.args.folders==='default'),
						click: function() { ui.args.folders='default'; gridLoad(lastLayoutMode)} },
					{	label: 'First', type:'radio', checked:(ui.args.folders==='first'),
						click: function() { ui.args.folders='first'; gridLoad(lastLayoutMode) } },
					{	label: 'Hidden', type:'radio', checked:(ui.args.folders==='hidden'),
						click: function() { ui.args.folders='hidden'; gridLoad(lastLayoutMode) }  },
					{	label: 'Last', type:'radio', checked:(ui.args.folders==='last'),
					 click: function() { ui.args.folders='last'; gridLoad(lastLayoutMode) } }
					]
				},
			...
			submenu: [
				{ label: 'Descending', type:'checkbox', checked:(ui.args.descending===true), accelerator:'Alt+Shift+D',
					click: function(item, win, ev) {
						...
						click: function(item, win, ev) { ui.args.shuffle=item.checked; gridLoad(lastLayoutMode)	}
				},{	type: 'separator'
				},{
					label: 'File Date', type:'radio', checked:(ui.args.order=='date'),
					click: function() { ordMenu.items[MNUORDERSHUF].checked = ui.args.shuffle = false; ui.args.order='date'; gridLoad(lastLayoutMode) }
				},{
					label: 'File Name', type:'radio', checked:(ui.args.order=='name'),
					click: function() { ordMenu.items[MNUORDERSHUF].checked = ui.args.shuffle = false; ui.args.order='name'; gridLoad(lastLayoutMode) }
				},{
					label: 'File Size', type:'radio', checked:(ui.args.order=='size'),
					click: function() { ordMenu.items[MNUORDERSHUF].checked = ui.args.shuffle = false; ui.args.order='size'; gridLoad(lastLayoutMode) }
				},{
					label: 'File Type', type:'radio', checked:(ui.args.order=='type'),
					click: function() { ordMenu.items[MNUORDERSHUF].checked = ui.args.shuffle = false; ui.args.order='type'; gridLoad(lastLayoutMode) }
				},{ 	type: 'separator'
			},{
...
function folderSize(apath){
	...
	sizeable(apath, null, function callback(err, size, details) {
	  if (err){
	    console.error('folderSize(), sizeable error:', err)
//	      throw err;
		alert('folderSize(), sizeable error:\n'+err)
		return
	}
/*
	details = {
		...
		subfolders: {
			...
			{ folder: string,	size:int, subfolder:[] },
			...
		}
	}
*/
		let totalitems = 0;
		function iterate(subs){
			for(let item of subs){
				totalitems++
				if(item.subfolders){
					iterate(item.subfolders)
				}
			}
		}
		iterate(details.subfolders)

		let result = `<div style='padding:1em'>
			<b>${details.folder}</b><br>
			&nbsp; ${details.subfolders.length.toLocaleString()} items<br>
			&nbsp; ${totalitems} total items<br>
			&nbsp; ${ui.calc.bytesToStr(details.size)}<br>
			&nbsp; ${details.size.toLocaleString()} bytes<br>
		<br>`

		//get subfolders of apath
		let subs = []
	  for(let sub of details.subfolders){
			if(sub.folder){
				subs.push(sub)
				if(sub.subfolders){
					totalitems = 0;
					iterate(sub.subfolders)
					sub.totalitems = totalitems
				}
				else
					sub.totalitems = 0
			}
		...
		subs.sort((a,b) => {
		  let nameA = a.folder.toUpperCase(); // ignore upper and lowercase
	  	let nameB = b.folder.toUpperCase(); // ignore upper and lowercase
		  if (nameA < nameB) return -1;
		  if (nameA > nameB) return 1;
		  return 0;
		})
		let len = details.folder.length,
				tmp = ''
	  for(let sub of subs){
			let fldr = sub.folder.substring(len)
			tmp += `<tr>
					<td><b>${fldr}</b></td>
					<td>${sub.subfolders.length.toLocaleString()}</td>
					<td>${sub.totalitems.toLocaleString()}</td>
					<td>&nbsp; ${ui.calc.bytesToStr(sub.size)}</td>
					<td style='display:none'>${fldr}</td>	<!-- allow sort by folder name -->
					<td style='display:none'>${sub.size}</td>	<!-- allow sort by size -->
				</tr>`
		}
		if(tmp != ''){
			...
			result += `
				<style>
					#${tbid} tr td{ text-align:right;}
					#${tbid} tr td:first-child{ text-align:left;}
					#${tbid} tr:first-child td{ border:1px outset; background-color:#eee; cursor:pointer; text-align:center; min-width:5em;}
				</style>
				<table id="${tbid}"><tr>
					<td onclick="tableSort('${tbid}', 4, 1, false)">Folder</td>
					<td onclick="tableSort('${tbid}', 1, 1, true)">Items</td>
					<td onclick="tableSort('${tbid}', 2, 1, true)">&nbsp;Total Items</td>
					<td onclick="tableSort('${tbid}', 5, 1, true)">&nbsp;Total Size</td>
				</tr>
				${tmp}
				</table>
				<label style='font-size:0.8em; font-style:italic; '>Click column headers to sort</label>`
		}
		dlgFactory.msg(result+'</div>', 'Folder Size')
	});
...
function queFileMove(objlist, dir, cbAfterExec, cbBeforeExec){
	...
	let localcallback = function(job){
		...
		if(job.args.length > 0){
			console.log(`${job.msg} failed:`, job.args.toString())
			alert(`${job.msg} failed:\n${job.args.toString()}.`)
		}
		else {
			if(cbAfterExec)
...
function findItem(){
	let val, html, buttons, dlg

	val = (ui.var.findText != null ?ui.var.findText :'')

	html =`<label class="divBulkOptions"><input id=edFindText value='${val}' size=15></label>`
	buttons = {
		default: 'Find',
		"Find": function(dlg,btn){	//save and reverse slideshow
			let txt, ftxt, pos, itm, fnd

			txt = dlg.querySelector('#edFindText').value.trim()
			if(txt == '') return

			pos = (ui.var.findText === txt ?ui.var.findPos +1 :0)
			ftxt = txt.toLowerCase()
			fnd = false

			for(pos; pos < items.length; pos++){
				itm = items[pos]
				if(itm.basename.toLowerCase().indexOf(ftxt) >= 0){
					fnd = true
					break;
				}
			}
			...
			ui.var.findText = txt
			if(fnd===true) {
				dlgFactory.bbl(`"${txt}" found`)
				ui.var.findPos = pos
				itemSelect(itm.pid, true)
			}
			else{
				ui.var.findPos = -1
				dlgFactory.bbl(`Not found: "${txt}"`)
			}
		},
	}

	dlg = dlgFactory.create({
		attrib:{ className:'dlgBulk'},
		body:html,
		buttons:buttons,
		focusId:'#edFindText',
		title:'Find Folder Item',
	})
}