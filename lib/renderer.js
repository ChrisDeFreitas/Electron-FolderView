...
exports.sayhey = function() {
	...
	console.log('FolderView v', process.env.npm_package_version)
}
					...
					click: function(item, focusedWindow){
						if(dlgSysInfo===null){
							...
							var str = `
								...
								<br>
								<b>FolderView</b> v${process.env.npm_package_version}<br>
...
function exportToFile(){
	...
	<style>
		...
		#taList{border:1px solid silver; font-family:sans-serif; height:50vh; overflow:auto; text-overflow:ellipsis; width:100%; }
	</style>
	<div id=divOptions>
		&nbsp; Output:
		...
		<label><input onclick='dlgExport.dlg.listType(this,2)'  name=format type=radio>src</label>
		<br>
		&nbsp; Include:
	...
	dlgExport.dlg.include={'Line no':false, 'File ext':false, 'File path':false, 'File size':false , 'File date':false, 'isDirectory':false}
	dlgExport.dlg.listType = function(ctrl,atype){
		...
		function local_Format(){
			...
			if(opts.format=='src') return 'file:///'
		}
		...
		function local_CSV(idx, obj){
			...
			if(tmp != '') str += tmp
			tmp = (opts.include['File path']==false ?`"${local_Format() +obj.basename}"` :`"${local_Format() +obj.path}"`)
			if(tmp != '') str += (str=='' ?'' :',')+tmp
		...
		function local_JSON(idx, obj){
			...
			if(tmp != '') str += '  '+tmp
			tmp = (opts.include['File path']==false ?`"file":"${local_Format() +obj.basename}"` :`"file":"${local_Format() +obj.path}"`)
			if(tmp != '') str += (str=='' ?'  ' :',\n  ')+tmp
		...
		function local_List(idx, obj){
			var str =
				...
				+local_Format() +(opts.include['File path']==false ?obj.basename :obj.path)
		...
		function local_M3U(idx, obj){
			var str = local_Format() +(opts.include['File path']==false ?obj.basename :obj.path)
			return (html=='' ?'' :'\n')+str
		}
