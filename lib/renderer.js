				...
				click: function(item, win, ev) {
					...
					click: function(item, win, ev) {
						...
						dlgFactory.create({	//rename dlg
							...
							left:'38vw',
							width:'24vw',
							body: `<label style="display:block;margin-top:0.5em">Old name <br> <input id=inpOld type=text value="${fn}" style='width:88%; text-align:left;'> </label>
											 <label style="display:block;margin-top:0.5em">New name<br> <input id=inpRNNew type=text value="${fn}" style='width:88%; text-align:left;' tabindex=0>
										 <button class=dlgButton style="font-weight:bold" onclick="document.getElementById('inpRNNew').value='${fn}'">&curarr;</button> </label>
										 <label style="display:block;margin-top:0.5em"><input id=cbRNAutoload type=checkbox checked> Reload folder items after change</label>`,
							buttons:{
								...
								Rename: function(dlg, btn){	//save and close
									...
									if(fs.existsSync(newpath)===true){
										dlgFactory.msg(`Unfortunately, a file named "${newfn}" was found in this folder.`)
										return
									}
									dlgFactory.create({
										focusId: '#btn0',
										...
										buttons:{
											...
											Confirm: function(dlg, btn){	//save and close
												var autoload = dlg.querySelector('#cbRNAutoload').checked
												dlgFactory.close(dlg)
