	...
	window.addEventListener('contextmenu', (e) => {
		...
		if(lastLayoutMode == 'videoWall'){
			if(e.target.id == 'videoplayer'){
				template = [	
					{ label: 'Find Dupes', accelerator: 'Ctrl+F',
						click: function(item, focusedWindow){
							let obj = videoWall.itmByUid( player.lastTile.dlg.uid )
							require('./dlgFind').open({ 
								find:obj.basename, 
								subFolders:true 
							})
					...
					{ label: `Last ${videoWall.tiles.length} videos`, click: function(){ videoWall.tilesPrior() }	},
					{ label: `Next ${videoWall.tiles.length} videos`, click: function(){ videoWall.tilesNext() }	},
			...
			}else{  //video tile context menu
				template = [	
					{ label: 'Last Video',
						click: async function(item, win, ev) {
							await videoWall.tilePlayLast( e.target )
						}
					},
					{ label: 'Next Video',
						click: async function(item, win, ev) {
							await videoWall.tilePlayNext( e.target.dataset.tid )
						}
					},
					{	type: 'separator' },
					{ label: 'Copy', accelerator:'C',
						click: function(item, focusedWindow, event) {
							renderer.fileCopy(e.target.id)
						}
					},
					{	label: 'Delete', accelerator:'Delete',
						click: function(item, win, ev) {
							// var obj = controlIdToObj(target.id)
							let obj = videoWall.itmByUid( e.target.dataset.uid )
							fileDelete(obj)
						}
					},
					{ label: 'Find Dupes', accelerator: 'Ctrl+F',
					...
					{ label: 'Move', accelerator:'M',
						click: function(item, focusedWindow, event) {
							fileMove(e.target.id)
						}
					},				
				{	type: 'separator' },
					{ label: `Last ${videoWall.tiles.length} videos`, click: function(){ videoWall.tilesPrior() }	},
...
async function fileDelete(obj){		//delete item from disk and remove from grid
	...
	if(gallery===null && obj.mediaType === 'image'){
		...
		await local_execFunc()

	async function local_execFunc(){
		if(!confirm(`Delete:\n"${obj.path}"`)) {
		...
		if(lastLayoutMode == 'videoWall'){
			await videoWall.tilePlayNext( obj.tid )
		}
...
function fileMove(controlID,successFunc){
	...
	bar.show({
		...
		onSelect: function(dir){
			...
			let cbBeforeExec = null
			if(lastLayoutMode == 'videoWall'){
				cbBeforeExec = async function(job){
					await videoWall.tilePlayNext( obj.tid )				
				}
			}
			...
			queFileMove(obj, dir, cbAfterExec, cbBeforeExec )
		}
	})
	if(gallery===null && obj.mediaType === 'image'){
		galleryShow(obj.pid)
