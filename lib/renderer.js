	...
	var template = [
	  {
	    ...
	    submenu: [
	        ...
	        label: 'Memory',
					click: function(item, focusedWindow){
						var str = JSON.stringify(process.getSystemMemoryInfo()).replace(/,/g, '<br>').replace(/{/, '<b>System</b><br>').replace(/}/g, '<br><br>')
							+ JSON.stringify(process.getProcessMemoryInfo()).replace(/,/g, '<br>').replace(/{/, '<b>Process</b><br>').replace(/}/g, '')
						dlgFactory.msg(str, 'Memory Usage')
					}
				},{
	        label: 'Reload',
					click: function(item, focusedWindow){
