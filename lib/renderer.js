...
//var mnuScale1=null		//failed: can not dynamically change menuitem.label
...
exports.mainMenuGen = function(exts, layoutmode) {
	var template = [
	  {
	    ...
	    submenu: fltSubmenuGen(exts, false)
			...
			label: 'Slideshow',
			submenu: [
				{ label: 'Hide', accelerator: 'Escape',
	        click: function() {
						if(gallery==null) return
						gallery.close()
					}
				},{ label: 'Zoom In', accelerator: 'Ctrl+Up',
	        click: function() {
						if(gallery==null) return
						var sc = gallery.getZoomLevel()
						sc += 0.1
						gallery.zoomTo(sc, {x:gallery.viewportSize.x/2*scale,y:gallery.viewportSize.y/2*scale}, 0)
					}
				},{
					label: 'Zoom Out', accelerator: 'Ctrl+Down',
	        click: function() {
						if(gallery==null) return
						var sc = gallery.getZoomLevel()
						sc -= 0.1
	...
	//failed: can not dynamically change menuitem.label
