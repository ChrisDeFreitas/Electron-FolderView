...
var dlgBulkDest=''		//last dest folder used
var mainmenu = null
	...
	var template = [
	  {
	    ...
	    submenu: [
        	...
        	type: 'separator'
      	},{ label: 'Bulk Copy/Delete/Move', accelerator: 'F7',
...
function fileRename(objid){
	...
	if(typeof ui.var.renamedLast==='undefined'){ //only set in this func
		...
		ui.var._FilenameCopy = function(event){	//called by rename dlg
			const {clipboard} = require('electron')
	...
	dlgFactory.create({	//rename dlg
		...
		body: `<label style="display:block;margin-top:0.5em">Old name <br> <input id=inpOld type=text value="${fn}" readonly style='background:#ddd; color:#000; border:1px solid silver; width:25em; text-align:left;'> </label>
					 <label style="display:block;margin:0.5em 0 0.2em 0">New name<br> <input id=inpRNNew type=text value="${fnnew}" style='width:25em; text-align:left;' tabindex=0> </label>
					 <button class='el el-plus dlgButton' onclick="ui.var._FilenameInc(event)" title="Increment the new filename"></button> </label>
					 <button class='el el-minus dlgButton' onclick="ui.var._FilenameDec(event)" title="Decrement the new filename"></button> </label>
					 <button class='el el-plus-sign dlgButton' onclick="ui.var._FilenameLoad(event)" title="Load and increment the last filename used"></button> </label>
					 &nbsp; &nbsp;
					 <button class='el el-refresh dlgButton' onclick="document.getElementById('inpRNNew').value='${fn}'" title="Reset new filename"></button>
					 <button class='el el-arrow-down dlgButton' onclick="ui.var._FilenamePaste(event)" title="Paste text from clipboard"></button> </label>
					 <button class='el el-arrow-up dlgButton' onclick="ui.var._FilenameCopy(event)" title="Copy text to clipboard"></button> </label>
...
function bulkOps(pid){
	...
	for(key in items){
		...
		if(gExtFilter!=null && gExtFilter[obj.type]===undefined) continue
		list += `<label id=lbl${obj.pid}><input onclick='dlgBulk.dlg.itemToggle(this)' type=checkbox id=cb${obj.pid}> ${obj.basename}</label>\n`
	}
	...
	dlgBulk = dlgFactory.create({
		...
		attrib:{ className:'dlgBulk', style:{zIndex:900, maxWidth:'50em'}},
		body: `<style>
			#divBulkOptions{border-bottom:1px #000 solid; color:silver; font-weight:bold; line-height:2em; width:100%; }
			...
			#divBulkList{background:white; margin:0; max-height:40vh; overflow: auto; }
			#divBulkList label{display:block; }
			#divBulkList label input{ vertical-align:middle; }
			</style>
			<div id=divBulkOptions>
				&nbsp; <span style='color:#ffaf00; '>Destination:</span>
				<label title='Select destination folder'><input id=inpBulkDest value='${dlgBulkDest}' type=text> <button onclick='dlgBulk.dlg.destFolderGet()'>...</button></label>
				<br>&nbsp; <span style='color:#ffaf00; '>Select:</span> &nbsp; &nbsp;
				<!--label title='Show and sort by file date'><input onclick='dlgBulk.dlg.sort("date")' type=checkbox>File date</label>
				<label title='Show and sort by file size'><input onclick='dlgBulk.dlg.sort("size")' type=checkbox>File size</label-->
				<!--br>&nbsp; <span style='color:#ffaf00; '>Select:</span-->
			</div>
			<div id=divBulkList>
				${list}
	...
	dlgBulk.dlg.itemToggle = function(cb){
		...
		if(cb.checked){
			...
			itemSelect(pid)
			if(ctrl != null) ctrl.scrollIntoView()
	...
	dlgBulk.dlg.exec = function(type){	//save and close
		...
		for(let obj of list){
			...
			if(type==='Copy'){
				...
				else // /E causes error when copying to subfolder: "Cannot perform a cyclic copy"
