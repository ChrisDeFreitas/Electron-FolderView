...
videoWall.drawDlg = function(){
	...
	dlgVideoWall = 	new CDialog({
		buttons:{
			...
			btnMute:{
				caption:'<input id=cbMute readonly=true type=checkbox style="margin:0; height:11px; top:2px; position:relative;"> Mute',
				title:'Mute all videos',
				onClick: function(dlg, btn){
					videoWall.muted = !videoWall.muted

					player.mute( videoWall.muted )
					this.querySelector('#cbMute').checked = videoWall.muted

					if(videoWall.muted == false) return
	...
	btn = dlgVideoWall.querySelector('#btnMute')
	btn.style.width = '8em'
	...
	elCreate: function(){
		...
		this.el.addEventListener('click', function(event){
			...
			if(player.lastTile != null){
				...
				player.lastTile.muted = videoWall.muted
			}
			return ui.eventStop(event)
		})
