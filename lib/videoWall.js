...
var videoWall = {
	...
	tileSwap: async function(tid){		//load new video into tile, called from renderer.js
		let tile = document.querySelector('#vid'+tid)
		let result = await tile.dlg.swap()
		if(result == -2){	//no more video files; empty tile to allow delete/move on file
			await tile.pause()
			tile.dataset.uid = null
			tile.dlg.uid = null
			tile.src = ''
			tile.title = 'No more videos to load'
		}
	},
	tileUpdate: async function( itm, tile = null ){		//refresh itm props in tile
		...
		tile.dataset.uid = itm.uid
		tile.dlg.uid = itm.uid
		tile.dlg.timeTotal = 0			//reset
...
videoWall.drawDlg = function(){
	...
	let vidcount = (this.tiles ?this.tiles.length +this.npq.list.length :0),
	...
	for(var idx in items){	//create video contaners
		...
		if(++tid < maxitems) {
			...
			html += `<video controls muted=true preload='metadata' class='grid-item videoWall'
				...
				data-uid="${itm.uid}" data-uidlast="-1" data-tid="${tid}"
