...
function grindArgv(argmap, argvPreprocessCb, firstArg = 2){
	let args = process.argv.splice(firstArg)
...
function grind(args, aargmap){
	...
	function local_parseOp(op, arg, idx){		//is arg in format: arg{op}val
		...
		if(ii === arg.length-1){				//extra space in op format: arg{op} val
			...
			let pos = arg.indexOf(op)
			val = arg.substring( pos +1 )	
			arg = arg.substring( 0, pos )
			// let arr = arg.split(op, 2)
			// arg = arr[0]
			// val = arr[1]
			// err('local_parseOp', op, arg, val)
		}
...
function typeConvert(arg, type){
	...
	case 'json':
		try{
			...
			//err(error)
			return 'grinder.typeConvert() error converting JSON for: ['+arg+']: '+error
		}
	}
	return arg
}
