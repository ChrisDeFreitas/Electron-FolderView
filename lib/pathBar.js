 function pathBar(){
	...
	this.show = function(options){
		...
		this.dlg = dlgFactory.create({
			...
			style:{width:'40em', paddingBottom:'1em', zIndex:1600},
			body: this.html(),
			...
			|| nm==='Recent'){
				...
				btnlist = dlgFactory.btnList({		//returns div container
					...
					,	onSelect: function(btn, path, idx, list){
							...
							if(path==='Copy Path'){
								renderer.clipboardWrite(ui.calc.pathForOS(ui.var.pathBarPath))
								return
							}
							if(path==='Copy src'){
								var src = 'file:///'+ui.var.pathBarPath.replace(/\\/g,'/')
...
//							console.log('pathBar menu select:', nm)
	...
	this.folderLoad = function(url, msg, showitems){
		...
		for(var key in arr){
			var str = arr[key]
			if(str[str.length-1]=='/') 
				arr[key] = str.substr(0, str.length-1)
		}
...
//			if(isfile	|| folderitems.length==0)
		...
		var dlgfldrnames = dlgFactory.btnList({
			...
			,	onSelect: function(btn, nm, idx, list){
					...
					var nmlen = nm.length
						, idx = ui.var.pathBarPath.indexOf(nm)
					if(idx < 0) {
						dlgFactory.msg(`Selected folder, "${nm}", is not available for download.`,'Warning')
						return
					}
					if(idx == (ui.var.pathBarPath.length -nmlen -1)) { //this is the folder whose contents are displayed
					...
					var newpath = ui.var.pathBarPath.substr(0,idx +nmlen +1)
	...
	this.folderItemsShow = function(btnlast, folderitems, showitems){
		...
		this.dlgfolderitems = dlgFactory.btnList({
			...
			,	onSelect: function(btn, nm, idx, list){
					...
					var newpath = ui.var.pathBarPath +nm
	...
	this.folderItemsSort = function(folderItems){
		/* var obj = {
			basename: fn,
			date:stat.mtime,
			size:stat.size,
		...
		var newItems = folderItems.sort(function(a, b){
			let aa = a['sortname'], bb = b['sortname']
			if(aa < bb) return -1
			if(aa > bb) return 1
			return 0
		})
	...
	this.save = function(){
		this.btnResult = true
		this.options.onSelect(ui.var.pathBarPath)
		dlgFactory.close(this.dlg,true)
	}
}
pathBarFolderNew = function(){	//create new folder
	renderer.folderNew( ui.var.pathBarPath, function( dir){
		if(dir===false) return
